...
def run_direct(
    ...
    fieldname='t2m', 
    unit='K', 
    input_dir='/home/tsternal/WeatherExtremes2/data/michael_t2_mean/',
    output_dir='/home/tsternal/WeatherExtremes2/data/compare_output/', 
    prefix='T2MEAN', 
    ndays = 0, 
    aggmode = 'max', 
    y_0=1966, 
    ...
    if calcmode == 'percentile' or calcmode == 'compute':
        ...
        for mm in analysis_months:
            ...
            print(f"Processing days of month {str(mm)}")
            for dd in tqdm(list(range(1, nday_per_month[mm - 1] + 1))):
                index = 0
                data_field = None
                data_keep = None
                ...
                '''
                if save_datafields:
                    np.save(f"{direct_data_fields_dir}/day_{dd-1}", data_field)
                if with_seasonality:
